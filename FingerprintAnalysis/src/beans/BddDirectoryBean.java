        ...
        for (File file: listOfFiles) {
        	if (file.isFile()) {
                if (file.getName().endsWith(".png") || file.getName().endsWith(".jpg") || file.getName().endsWith(".jpeg")) {
                	Terminal.executeCommand(executableMindtctString + " -m1 " + file.getAbsolutePath() + " " + getFileNameWithoutExtension(file));
            		Terminal.executeCommand("rm *brw *dm *hcm *lcm *lfm *min *qm");		
            		System.out.println("Generating .\\trash\\" + getFileNameWithoutExtension(file) + ".txt");
            		FileConvertor.convertIt(file.getAbsolutePath(), getFileNameWithoutExtension(file));
